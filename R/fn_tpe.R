...
generateClimate2 <- function(lon, lat, rcp, year, yearNb, modelNb, path,
                            pathCLI=NA, seed=1337) {
  if(is.na(pathCLI)) {
    pathCLI <- path
  }
  ...
  #Extract data from worldclim
  coordinates <- cbind(lon,lat)
  grid.tmin <- geodata::worldclim_tile("tmin", lon = lon, lat = lat,
                                      path = tempdir())
  grid.tmax <- geodata::worldclim_tile("tmax", lon = lon, lat = lat,
                                      path = tempdir())
  grid.prec <- geodata::worldclim_tile("prec", lon = lon, lat = lat,
                                      path = tempdir())
  tmin <- terra::extract(grid.tmin,coordinates)
  tmax <- terra::extract(grid.tmax,coordinates)
  prec <- terra::extract(grid.prec,coordinates)
  ...
  if(!is.na(tmin[1])) {
    ...
    mdata[,1] <- seq(1:12)
    mdata[,2] <- rep(-99,12)
    mdata[,3] <- as.numeric(tmax[1,])
    mdata[,4] <- as.numeric(tmin[1,])
    mdata[,5] <- as.numeric(prec[1,])
    mdata[,6] <- rep(-99,12)
    mdata[,7] <- rep(-99,12)
